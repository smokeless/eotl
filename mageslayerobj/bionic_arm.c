...
void extra_create()
{
    set( "id", ({ "arm", "bionic arm" }) );
    set( "short", "a bionic arm" );
    set( "long",
        "This device replaces your entire left arm.  Enhanced "
        "hydraulics offer greater strength potential, but it is "
        "less dextrous than natural flesh and quite ugly.  "
        "Unlike most implant "
        "devices, it does include a workable claw-type appendage "
...
void post_wear( object wearer )
{
    ...
    tell_object( wearer, "You socket the bionic arm into your shoulder "
        "implant.\n" );
        ...
        foreach( object temp : stuff )
                ...
                member( temp->query_areas(), "left hand" ) > -1 )
            {
                tell_object( wearer, "You must first remove all 'left hand' "
                    "armor covering your bionic arm.\n" );
                return 1;
            }
...
{
    if( query_worn() && arg == "str" )     //  15% bonus
        return ENV( THISO )->query_base_stat( arg ) * 3 / 20;
    if( query_worn() && arg == "dex" )     //  15% penalty
        return( - ENV( THISO )->query_base_stat( arg ) * 3 / 20 );
